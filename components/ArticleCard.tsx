    ...
    return (
        <>
            <div className={"justify-center flex flex-col text-center items-center"}>
                <Link href={`/articles/${title}`} className="relative aspect-square h-full  w-full flex flex-col justify-center items-center text-center center">
                    <Image layout="fill" objectFit="cover" src={splashImageUrl} alt={splashImageUrl}>
                    </Image>
                </Link>

                <Link href={`/articles/${title}`} className="hover:text-my-blue">{title}</Link>
